<?php
include_once (APP_PATH . '/components/elements/logo.php'); 
include_once (APP_PATH . '/components/elements/signInButton.php'); 
include_once (APP_PATH . '/components/sidebar/menuButton.php'); 
function sidebar() {
?>
    <div class="sidebar" id="sidebar">
        <div class="layout fixed w-full h-screen" onclick="closeSidebar(event)">
        </div>
        
        <div class="fixed panel h-screen">

            <?php 
                logo(classes: 'sidebar-panel-header', onMenuClick: 'closeSidebar(event)');
            ?>
            <div class="panel-content flex flex-row">
                <div class="bd flex flex-row menu-segment">
                    <?php 
                        $svg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                        </svg>';
                        $menuTitle = "Home";
                        
                        menuButton($svg, $menuTitle, '/');
                    ?>
                </div>

                <?php if (isset($_SESSION['user_id'])) : ?>

                    <div class="bd flex flex-row menu-segment">
                        <?php 
                            $svg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.91 11.672a.375.375 0 010 .656l-5.603 3.113a.375.375 0 01-.557-.328V8.887c0-.286.307-.466.557-.327l5.603 3.112z" />
                        </svg>';
                            $menuTitle = "My Videos";
                            
                            menuButton($svg, $menuTitle, '/myvideos');
                        ?>
                        <?php if ($_SESSION['is_admin']) : ?>
                            <?php 
                                $svg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m0-10.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75h-.152c-3.196 0-6.1-1.249-8.25-3.286zm0 13.036h.008v.008H12v-.008z" />
                            </svg>';
                                $menuTitle = "Takedowns";
                                
                                menuButton($svg, $menuTitle, '/takedowns');
                            ?>
                        <?php endif; ?>
                    </div>

                    <div class="bd flex flex-row menu-segment">
                        <?php 
                            $svg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>';
                            $menuTitle = "Profile";
                            
                            menuButton($svg, $menuTitle, '/profile');
                        ?>

                        <?php 
                            $svg = '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                        </svg>';
                            $menuTitle = "Log out";
                            
                            menuButton($svg, $menuTitle, onClick: "logout(event)");
                        ?>
                    </div>
                <?php endif; ?>
                
                <?php if (!isset($_SESSION['user_id'])) : ?>
                    <div class="flex flex-row menu-segment">
                        <span>Sign in to comment and create videos.</span>
                        <?php 
                            signInButton();
                        ?>
                    </div>
                <?php endif; ?>
            </div>

        </div>
    </div>
<?php
}